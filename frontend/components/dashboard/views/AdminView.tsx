...
function MachineRow({ machine, liveData, onSelect, selected }: {
  ...
  const statusColor = status === 'critical' ? C.red : status === 'warning' ? C.yellow : C.green;
  const statusBg    = status === 'critical' ? 'rgba(239,68,68,.12)' : status === 'warning' ? 'rgba(245,158,11,.12)' : 'rgba(16,185,129,.12)';
  ...
  return (
    ...
    >
      ...
      <div style={{ marginLeft: 'auto' }}>
        <span style={{ padding: '3px 10px', borderRadius: 99, fontSize: 10, fontWeight: 700, background: statusBg, color: statusColor }}>
          {status.toUpperCase()}
        </span>
      </div>
    </div>
...
export default function AdminView({ activeSection }: { activeSection: string }) {
  ...
  const machinesWithLive = machines.map(m => ({ ...m, ...(latestTelemetry[m.id] ?? {}) }));
    ...
    <div className="animate-fade-up" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ background: C.yBlue, color: '#fff', fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: 3, letterSpacing: .5 }}>
                CORE FEATURE
              </span>
            </div>
            <div style={{ color: '#4A6580', fontSize: 12, marginTop: 2 }}>
        ...
        <button style={{
          ...
          boxShadow: '0 4px 12px rgba(227,24,55,.35)',
        }}>
          Nexfloor Console →
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
        {[
          ...
          <div key={s.label} className="card" style={{ padding: '20px 22px', background: `linear-gradient(135deg, var(--card-bg), ${s.glow})` }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: C.g400, letterSpacing: .8, textTransform: 'uppercase', marginBottom: 8 }}>
              {s.label}
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 16 }}>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          <div style={{
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: C.g500, textTransform: 'uppercase', letterSpacing: .5 }}>
              Live Sensor Stream
            </span>
          ...
          <div style={{ display: 'flex', gap: 16, padding: '8px 16px', background: 'var(--g50)', borderBottom: `1px solid ${C.border}` }}>
            {['Machine', 'Temperature', 'Vibration', 'RPM', 'Power', 'Status'].map(h => (
              <div key={h} style={{
                fontSize: 10, fontWeight: 700, color: C.g400, letterSpacing: .4, textTransform: 'uppercase',
                width: h === 'Machine' ? 75 : h === 'Status' ? undefined : 80,
                marginLeft: h === 'Status' ? 'auto' : 0,
              }}>{h}</div>
            ))}
          </div>
            ...
            <div style={{ borderTop: `2px solid ${C.blue}`, background: 'var(--g50)', animation: 'fade-up .25s ease' }}>
              <div style={{
                ...
                <button
                  ...
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.g400, fontSize: 18, lineHeight: 1, padding: '0 4px' }}
                >✕</button>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', borderBottom: `1px solid ${C.border}` }}>
                {[
                  ...
                  <div key={item.label} style={{ padding: '12px 16px', borderRight: i < 3 ? `1px solid ${C.border}` : 'none' }}>
                    <div style={{ fontSize: 9, fontWeight: 700, color: C.g400, letterSpacing: .8, marginBottom: 2 }}>{item.label}</div>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 3 }}>
                      <span className="mono" style={{ fontSize: 20, fontWeight: 800, color: item.c }}>
                        {item.v != null ? Number(item.v).toFixed(item.label === 'VIBRATION' ? 3 : 1) : '—'}
                      </span>
                      <span style={{ fontSize: 10, color: C.g400 }}>{item.u}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
                {(['temperature', 'vibration', 'rpm', 'power_consumption'] as const).map((metric, i) => (
                  <div key={metric} style={{
                    ...
                    <div style={{ fontSize: 10, fontWeight: 700, color: C.g400, textTransform: 'uppercase', letterSpacing: .5, marginBottom: 6 }}>
                      {metric.replace('_', ' ')}
                    </div>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          ...
          <div style={{ padding: '8px 0' }}>
              ...
              <div key={a.id} style={{ padding: '10px 16px', borderBottom: `1px solid var(--g50)` }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  <div style={{
                    ...
                    <svg width="13" height="13" fill="none" stroke={a.severity === 'critical' ? C.red : C.yellow} strokeWidth="2" viewBox="0 0 24 24">
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: C.g400, marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {a.message}
                    </div>
                  </div>
                </div>
            ...
            {activeAlerts.length === 0 && (
              <div style={{ padding: 24, textAlign: 'center', color: C.g400, fontSize: 13 }}>
                ✓ No active anomalies
              </div>
            )}
          </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                color: selected === m.id ? C.blue : C.g500,
              ...
              <div key={item.label} style={{ padding: '16px 20px', borderRight: i < 3 ? `1px solid ${C.border}` : 'none' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                  <span className="mono" style={{ fontSize: 26, fontWeight: 800, color: item.c }}>
                    {item.v != null ? Number(item.v).toFixed(item.label === 'VIBRATION' ? 2 : 1) : '—'}
                  </span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
            {(['temperature', 'vibration', 'rpm', 'power_consumption'] as const).map((metric, i) => (
              <div key={metric} style={{
                ...
                <div style={{ fontSize: 10, fontWeight: 700, color: C.g400, textTransform: 'uppercase', letterSpacing: .5, marginBottom: 8 }}>
                  {metric.replace('_', ' ')}
                </div>
  ...
  if (activeSection === 'reports') return (
    <div className="animate-fade-up" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="card" style={{ padding: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 16 }}>
          {[
            ...
            <div key={field.id}>
              <label style={{ fontSize: 11, fontWeight: 700, color: C.g500, letterSpacing: .5, textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>
                {field.label}
              </label>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            ...
            style={{
              ...
              boxShadow: '0 4px 12px rgba(227,24,55,.3)',
            }}
          >
          ...
          <button
            ...
            style={{ padding: '10px 16px', borderRadius: 6, border: `1px solid ${C.g200}`, background: 'transparent', color: C.g500, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}
          >
            Refresh List
          </button>
        </div>
      ...
      <div>
        ...
        {reports.length === 0 ? (
          <div className="card" style={{ padding: 32, textAlign: 'center', color: C.g400 }}>
            ...
            {reports.map((r, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 18px', borderBottom: `1px solid ${C.border}` }}>
                <div style={{ width: 36, height: 36, borderRadius: 8, flexShrink: 0, background: 'rgba(59,130,246,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="16" height="16" fill="none" stroke={C.blue} strokeWidth="2" viewBox="0 0 24 24">
                ...
                <span style={{ padding: '3px 10px', borderRadius: 99, background: 'rgba(59,130,246,.12)', color: C.blue, fontSize: 10, fontWeight: 600 }}>
            ...
            {users.map(u => (
              <tr key={u.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                ...
                <td style={{ padding: '11px 16px' }}>
                  <span style={{ padding: '2px 8px', borderRadius: 99, background: 'rgba(59,130,246,.12)', color: C.blue, fontSize: 11, fontWeight: 600 }}>
                    {u.role.replace('_', ' ')}
                  </span>
                </td>
                <td style={{ padding: '11px 16px' }}>
                ...
                <td style={{ padding: '11px 16px', fontSize: 11, color: C.g400 }}>
                  {u.last_login ? new Date(u.last_login).toLocaleDateString() : 'Never'}
                </td>
                <td style={{ padding: '11px 16px' }}>
                  <button
                    onClick={() => api.admin.toggleUser(u.id).then(() => api.admin.users().then(setUsers))}
                    style={{ padding: '4px 12px', borderRadius: 6, border: `1px solid ${C.g200}`, background: 'transparent', cursor: 'pointer', fontSize: 11, color: (u as any).is_active ? C.red : C.green, fontWeight: 600 }}
                  >
